...
3. アプリを起動すると「"${APP_NAME}.app"は開いていません」という警告が表示されます
   （未公証アプリのため。以下の手順で最初の1回だけ許可してください）
   1. 警告のポップアップ右上の **？** をクリックし、Macユーザガイドを開く
   2. ポップアップの **「完了」** を押して閉じる
   3. **システム設定の「プライバシーとセキュリティ」** を開く
   4. 「セキュリティ」項目に表示された **「このまま開く」** をクリック
